...
public class DESUtil {
	...
	public static String encryptDes(String data,String DESkey,String transformer,byte[] iv) {
		...
		try {
			// DES算法要求有一个可信任的随机数源
			SecureRandom sr = new SecureRandom();
			DESKeySpec deskey = new DESKeySpec(iv);
			// 创建一个密匙工厂，然后用它把DESKeySpec转换成一个SecretKey对象
			SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DES");
			SecretKey key = keyFactory.generateSecret(deskey);
			// 加密对象
...
//		System.out.println(DESUtil.encryptDes("223344", "13e998492ad888e695a431c68ac28178"));
		...
		String encryptDESwithCBC = DESUtil.encryptDESwithCBC("中国人", "87654321");
		System.out.println(encryptDESwithCBC);
		System.out.println(DESUtil.decodeDESwithCBC(encryptDESwithCBC, "87654321"));
