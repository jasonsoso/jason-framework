...
public class RSAUtil {
	...
	public static boolean verify(byte[] data, byte[] publicKey, String sign) {
		byte[] signBytes = EncodeUtils.decodeBase64(sign);
	...
	public static byte[] encrypt(byte[] data, String publicKey) {
		byte[] pubKey = EncodeUtils.decodeBase64(publicKey);
	...
	public static byte[] decrypt(byte[] data, String privateKey) {
		byte[] priKey = EncodeUtils.decodeBase64(privateKey);
	...
	public static byte[] decrypt(String data, PrivateKey privateKey) {
		try {
			...
			Cipher cipher = Cipher.getInstance(RSA);
			cipher.init(Cipher.DECRYPT_MODE, privateKey, secureRandom);
			byte[] encrypt = EncodeUtils.decodeBase64(data);
...
//	}
	...
	public static String decodes(byte[] data, byte[] publicKey) {
		try {
			...
			cipher.init(Cipher.DECRYPT_MODE, pubKey, secureRandom);
			byte[] decrypt = cipher.doFinal(data);
			return EncodeUtils.encodeBase64(decrypt);// base64还是UTF-8，以后再确定吧
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	public static String decodes(String data, byte[] publicKey) {
		return new String(decode(data, publicKey), UTF8);
	}
	...
	public static void main(String[] args) {
		/** "公钥加密——私钥解密" RSA加密的私钥，用于解密，保存本地，不可泄露 */
		String prikeyvalue_password = "";
		
		/** "公钥加密——私钥解密" RSA加密的公钥，用于加密，保存于客户端 */
		String pubkeyvalue_password = 
			"MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC9zaQcTwgOo+CR17933ABB6nSd"+
			"DYfh1Jy5JQ5CA7RAVsCGau64onSZvqxMyTzNWKeGmm1OxFat4VVsKQXCDktvCR3X"+
			"lu5fePEpJY9BT39P3pC5XX3aQjJ3snAyqg9KMyyRuqqat8p3NaVis9W5GoN6mtEZ"+
			"K1LToJozLcY7/gmQfQIDAQAB";
		String aaa = encrypts("jason".getBytes(), pubkeyvalue_password);
		System.out.println("加密:"+aaa);
		
		String jsonStr = decrypts(aaa, prikeyvalue_password);
		System.out.println("解密："+jsonStr);
	}
}
